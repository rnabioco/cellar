...
Here is a list of the packages that will be installed. 
...
pkgs <- list("CRAN" = cran_req_packages,
             "Bioconductor" = bioc_req_packages,
             "Github" = git_req_packages) %>% 
  map(~tibble(package = .x)) %>% 
  bind_rows(., .id = "source")
