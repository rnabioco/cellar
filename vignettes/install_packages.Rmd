...
Before class, make sure this RMD file knits, which installs and double checks for the required packages covered in class. [Just press the Knit button on top!] Installation of some packages will require answering prompted questions, so run the document interactively prior to tryint to knit.  
...
# Linux users may need to install libssl-dev, libcurl4-openssl-dev, and libxml2-dev (using apt, pacman, etc.) to successfully install these packages in R. Package names may differ by linux distribution (these are the Debian package names).
s
...
## check that R packages load correctly

```{r load}
lapply(c(cran_req_packages, 
         bioc_req_packages, 
...
```{r}
modules_fullname <- c("umap-learn", "louvain")
modules_shortname <- c("umap", "louvain")
