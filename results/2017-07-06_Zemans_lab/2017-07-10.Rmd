---
title: "Zeman's lab single cell RNA-Seq expt.1"
...
sample_paths <- file.path("~/Projects/10x_data/runs/20170623_zeemans/results", 
                          samples, "outs/filtered_gene_bc_matrices/mm10")
      
names(sample_paths) <- c("1", "2", "3", "5", "4")
...
atcells <- Setup(atcells, min.cells = 5, min.genes = 250, do.logNormalize = T, total.expr = 1e4, project = "Zemans", names.field = 1, names.delim = "_")
...
atcells <- SubsetData(atcells, subset.name = "nUMI", accept.high = 40000)
...
atcells_markers %>% group_by(cluster) %>% top_n(10, avg_diff) -> top10
```
...
atcells_markers %>% 
  dplyr::filter(cluster == "7") -> interesting_cluster
...
knitr::kable(tf_info[1:10, ], caption = c("TFs found enriched in cluster 7"))
```
...
```{r}
atcells <- SetIdent(atcells, ident.use = atcells@data.info$res.0.5)
injured_type_II <- FindMarkers(atcells, c(0, 1), c(2, 3), only.pos = T)
```


* Genes enriched in the TD cell cluster that also shares markers with ATI cells  
```{r}
td_type1_like <- FindMarkers(atcells, 7, c(0, 1, 2, 3, 6, 8), only.pos = T)
```

* Genes enriched in the TD cell cluster that does not have markers of ATII cells (compared to cluster above and all other ATII cells)

```{r}
td_type2_like <- FindMarkers(atcells, 6, c(0, 1, 2, 3, 7, 8), only.pos = T)
```

* Genes enriched in the TD cell clusters compared to all other ATII clusters

```{r}
td_type1type2 <- FindMarkers(atcells, c(6, 7), c(0, 1, 2, 3, 8), only.pos = T)
```

* Genes enriched in the TD cell cluster that also shares markers with ATI cells, but not found in other type II population or type I.

```{r}
td_type1_non_type1 <- FindMarkers(atcells, c(7), c(0, 1, 2, 3, 4, 8), only.pos = T)
```

```{r}
outs <- list(
  injured_type_II = injured_type_II,
  td_type1_like = td_type1_like,
  td_type2_like = td_type2_like,
  td_type1type2 = td_type1type2,
  td_type1_non_type1 = td_type1_non_type1
)

walk2(outs, names(outs), 
     ~write.table(.x, paste0(.y, "_expression.txt"), 
                  sep = "\t", quote = F))
```
