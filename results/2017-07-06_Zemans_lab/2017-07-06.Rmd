...
atcells <- Seurat::Read10X("~/Projects/10x_data/runs/20170623_zeemans/results/3161-ATII-/outs/filtered_gene_bc_matrices_mex/mm10/")
...
atcells <- Setup(atcells, min.cells = 5, min.genes = 250, do.logNormalize = T, total.expr = 1e4, project = "Zemans", names.field = 2, names.delim = "-")
...
p <- map2(p, c(names(cell_ids), "all_samples"),
            ~.x + 
            coord_cartesian(c(-45, 45), c(-45, 55)) + 
            labs(title = .y) +
...
```{r save_objs, eval = T}
saveRDS(atcells, "analyzed_seurat_object.rds")
```
...
atcells_markers %>% 
  dplyr::filter(cluster == "8") -> interesting_cluster
...
knitr::kable(tf_info[1:10, ], caption = c("TFs found enriched in cluster 8"))
```
