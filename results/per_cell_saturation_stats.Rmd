...
Here's a quick analysis of 
```{r mod_10x_dat, eval = T}

d <- dat %>% mutate(seqs = decompress_sequences(barcode, 16))

d %>% group_by(barcode) %>%  summarize(total_umis = n(), total_reads = sum(reads)) -> dat_stats

dat_stats %>% 
  mutate(dup_rate = 1 - (total_umis / total_reads)) %>%  
  arrange(desc(total_umis)) %>% 
  ungroup() %>%  
  slice(1:1091) -> dat_stats

ggplot(dat_stats, aes(dup_rate)) + geom_density()

ggplot(dat_stats, aes(row.names(dat_stats), dup_rate)) + 
  geom_point()
```