...
dplyr::filter(cf_annotated,
              !is.na(`CSPA category`)) %>% 
  dplyr::select(p_val:gene,
         CD,
         `CSPA category`,
         UP_Protein_name,
         `# predicted TM domains`:`UniProt cell surface`) -> cf_annotated
```
...
filter_cell_surface <- function(df, cell_surface_markers) {
  df <- df %>% 
  ...
  dplyr::filter(cf_annotated,
                !is.na(`CSPA category`)) %>% 
    dplyr::select(gene:pct.2,
           CD,
           `CSPA category`,
           UP_Protein_name,
           `# predicted TM domains`:`UniProt cell surface`)
}
map(tds, ~filter_cell_surface(.x, cell_surface_markers)) -> tds_annotated
...
There is much more data to analyze to figure out what would be the best set of candidates, but here is a quick look at a few markers:
```{r}
cool_genes <- c(
  "Anxa2",
  "Emp2",
