---
title: "Zeman's lab single cell RNA-Seq analysis (combined expt 1 and 2)"
...
sample_paths_exp1 <- file.path("~/Projects/10x_data/runs/20170623_zeemans/results", 
...
sample_paths_exp2 <- file.path("~/Projects/10x_data/runs/20170728_zeeman_expt2/results", 
                          samples, "outs/filtered_gene_bc_matrices/mm10")
...
ids <- atcells@data.info$orig.ident
...
sample_names <- unique(atcells@data.info$orig.ident)
...
expt1_files <- dir("~/Projects/10x_data/runs/20170623_zeemans/results/gfp/",
    pattern = "*.txt", full.names = T, recursive = T)
expt2_files <- dir("~/Projects/10x_data/runs/20170728_zeeman_expt2/results/gfp/",
    pattern = "*.txt", full.names = T, recursive = T)
...
atcells <- MeanVarPlot(atcells, 
                    ...
                    fxn.y = logVarDivMean, 
                    x.low.cutoff = 0.0125, 
                    x.high.cutoff = 4, y.cutoff = 0.5, do.contour = F, do.plot = T)
...
```{r plot_pca, eval = T}
PCAPlot(atcells, 1, 2, cols.use = brewer.pal(10, "Paired"), 
        group.by = "sample_names")
PCElbowPlot(atcells)
```
...
### res 0.5  
```{r find_markers0.5, fig.width = 10, fig.height = 10, warning=FALSE}
...
### res 0.8
```{r find_markers0.8, fig.width = 10, fig.height = 10, warning=FALSE}
atcells <- FindClusters(atcells, 
                     pc.use = 1:15, 
                     k.param  = 25, 
                     resolution = 0.8, 
...
### res 1.2
```{r find_markers1.2, fig.width = 10, fig.height = 10, warning=FALSE}
atcells <- FindClusters(atcells, 
                     pc.use = 1:15, 
                     k.param = 25, 
                     resolution = 1.2, 
...
write_tsv(atcells_markers, "res0.8_markers.txt")
```
...
write_tsv(atcells_markers, "res0.8_merged_markers.txt")
```
