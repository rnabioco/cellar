---
title: "Jordan lab AML-PBMC single cell RNA-Seq expt.2"
...
Sample | Treatment
-------- | ----------  
Sample 1 | Pbmcs from a patient at Day 0 of treatment
Sample 2 | Pbmcs from a patient at Day 2 of treatment
Sample 3 | Pbmcs from a patient at Day 4 of treatment


```{r Jordan_data, eval = T}
pbmc <- Seurat::Read10X("~/Projects/10x_data/runs/AML/outs/filtered_gene_bc_matrices_mex/GRCh38/")
```
...
samples_lookup <- c("Day0", "Day2", "Day4")
...
## PCA
  To get an overview of the relationships between different cell types, PCA will be used first. Shown below is a plot of the first two prinicipal components computed using highly variable genes (approx ~1800 genes). The first two componenets of the PCA rarely capture all of the variation in the data, as can be seen in the second plot.  
...
```{r,  fig.width = 10, fig.height= 10}
aml_markers = c(Blasts_and_Immature_Ery_1 = "CD34", early_erythroids = "GATA2", Immature_Ery_2 = "GATA1", Immature_Ery_3 = "TFRC", Not_Immature_Granulocytes = "FCGR3A", Immature_Granulocytes_2 = "CXCL8", Erythroids = "HBA1")
...
-------- | ----------  | ------------  
0 | CD8 cytotoxic T-Cells | CD8A
1 | AML - Blasts Immature Erythroids/Granulocytes | CD34 High, lost after treatment
2 | CD4 T-Cells | CD3D and not CD8A 
3 | T-cells | Il-7
4 | NK cells, Monocytes | FCGR3A/CD16 FCER1G
5 | B-cells IGD naive?| CD79A CD19 IGHD
6 | Megakaryocyte | PF4
7 | AML - Early Erythroids | GATA1 / GATA2, lost after treatment
8 | B-cells IGA producing | CD79A IGHA1 
9 | Macrophages | S100A8
10 | Unknown | proliferative (PCNA, TK1, )
11 | Erythroids | HBA1
...
```{r, fig.width = 8, fig.height= 20 , eval = T}
...
titles <- c("Day 0", "Day 2", "Day 4")

p <- map2(p, titles,
            ~.x + 
            coord_cartesian(c(-45, 45), c(-60, 40)) + 
            labs(title = .y) +
...
## Interesting Observations
  Amongst the most enriched genes in the largest AML cluster (1) are a handful of LincRNAs (SNHG7, ZFAS1, and RP11-620J15.3)
