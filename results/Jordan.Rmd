...
Sample | Treatment
-------- | ----------  
Sample 1 | Pmbcs from a patient at AML diagnosis
Sample 2 | Pmbcs from a patient at remission
Sample 3 | Pbmcs from a patient at relapse

"The three samples are the same patient- sample 1 is diagnosis, sample 2 is remission and sample 3 is relapse. There are some immunophenotypic changes that we see- appearance of CD56 on malignant cells and some genetic changes – appearance of a CEBPA mutation. The hope is the RNA seq will pick out the malignant cells and there will be a disappearance/replacement with lymphocytes(remission) and a re-emergence(relapse)."
...
knitr::kable(cell_counts(pbmc),
             caption = "Number of cells passing a filter of needing at least 250 genes detected (>0 UMIs per gene) and each gene being present in at least 5 cells")
```
...
pbmc <- RunTSNE(pbmc, dims.use = 1:14, do.fast = T, k.seed = 1)
```
...
PCAPlot(pbmc, 1, 2)
PCElbowPlot(pbmc)
PCHeatmap(pbmc, pc.use = 1:14, cells.use = 2000, do.balanced = TRUE, label.columns = FALSE, use.full = FALSE, do.plot = F)
```
...
## Marker genes enriched in each cell population
...
TSNEPlot(pbmc, colors.use = brewer.pal(n = 11, "Paired"), do.label = T, label.size = 8)
```
...
```{r genes_enriched_per_cluster, fig.width = 10, fig.height= 10, eval = F }
top6_genenames <- split(top10, top10$cluster) %>% map(., ~.x[1:3, ]$gene)

plot_tsnes <- function(x, y) {
  #par(mar=c(0,0,1,0))
  #plot(0,type='n',axes=FALSE,main = y, ylab = "", xlab = "") 
  print(y)
  FeaturePlot(pbmc, x, pt.size = 0.2)
} 
...
```{r}
FeaturePlot(pbmc, "CEBPA")
```

```{r}
immune_markers <- c(t_cells = "CD3D", cd8_t ="CD8A", cytotoxic_t = "NKG7", dendritic = "FCER1A", "S100A8", megakaryocyte = "PF4", b_cell = "CD79A", cd4_4 = "IL7R", monocyte = "CD14")
FeaturePlot(pbmc, immune_markers)
```

Classification taken from (http://www.nature.com/articles/ncomms14049#f5)
```{r}
aml_markers = c(Blasts_and_Immature_Ery_1 = "CD34", early_erythroids = "GATA2", Immature_Ery_2 = "GATA1", Immature_Ery_3 = "TFRC", Immature_Granulocytes = "AZU1", Not_Immature_Granulocytes = "FCGR3A", im_gran_2 = "CXCL8", diff_2 = "HBA1")

FeaturePlot(pbmc, aml_markers)
```
...
```{r}
TSNEPlot(pbmc, colors.use = brewer.pal(n = 3, "Paired"), group.by = "orig.ident")
```

Cluster | Observation | Classification  
-------- | ----------  | ------------  
0 | AML - Blasts and Immature Erythroids | CD34 and GATA2, Mostly found at diagnosis  
1 |  Monocytes, Macrophage cells | CD14, FCN1, S100A8  
2 | AML - Immature Granulocytes | AZU1, Mostly found at diagnosis  
3 | AML - Early Erythroids | CD34 high,  Mostly found at relapse  
4 | CD8 cytotoxic T-Cells | CD8A, NKG7  
5 | AML - mature erythroid cells | HBA1,  Mostly found at relapse  
6 | CD4 T-Cells | CD3D and not CD8A   
7 | AML ? | ?, Mostly found at diagnosis  
8 | AML ? | IL8 (Cxcl8) , Mostly found at diagnosis  
9 | Dendritic | FCER1A  
10 | Megakaryocytes | High PF4  

```{r cluster_rename, eval= T, fig.width = 8, fig.height= 8 }
new.cluster.ids <- c(
...
p <- map2(p, titles,
            ~.x + 
            coord_cartesian(c(-40, 30), c(-40, 40)) + 
            labs(title = .y) +
